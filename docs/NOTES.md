...
来源：那是上一版加的**根目录切换栏**。打开 `/tmp/pvs-edit-xxxx/scratch.md` 这类临时文件时，
主进程会把文件所在目录注册成根目录（`pvs://` 授权需要），切换栏把它当成一个「根目录」显示了出来；
更糟的是当时树的根目录按「路径最短」挑，`/tmp/pvs-edit-x` 比项目根还短，文件树直接跳到了 /tmp。

修法：根目录区分**声明**（启动目录 / `--root` / 📂）与 **auto**（打开文件时顺带注册），
切换栏只列声明的那类、且少于两个就整条隐藏；树的根目录取「第一个声明的根目录」，
不再按路径长短挑。`listRoots()` 带 `auto` 标记，验收里也加了对应断言。

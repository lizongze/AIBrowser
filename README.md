...
| `pvs serve [--gui]` / `pvs stop` / `pvs status` | 常驻服务（`--gui` 开面板窗口，否则纯无头）；`--native-ua` 保留 Electron 原始 UA；默认拉起即返回（不等就绪，避免调用方的 shell 被挂住），`--wait` 则等到就绪再返回 |
...
- 任何时刻只有一个「控制器」进程持有控制通道，应用是**单实例**：重复启动会自己退出（并把已有窗口带到前台），不会顶掉正在服务的那个；想换模式（面板 ↔ 无头）用 `pvs serve --gui` / `pvs serve`，它会把不对的先停掉。

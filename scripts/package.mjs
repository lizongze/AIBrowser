...
async function writeCliShim(appDir, platform) {
  ...
  const lines = platform === 'win32'
    ? [
      ...
      'REM The first real command also starts the service (see :lazy_start); nothing runs at install time.',
      'setlocal',
      'set "HERE=%~dp0"',
      'set "EXE=%HERE%AIBrowser.exe"',
      'set "APP=%HERE%resources\\app.asar"',
      'if not exist "%HERE%resources\\app.asar" set "APP=%HERE%resources\\app"',
      'REM %1 must be captured here: inside a called label it would be empty',
      'set "PVS_FIRST=%~1"',
      'call :lazy_start',
      'set "ELECTRON_RUN_AS_NODE=1"',
      'set "AIBROWSER_PACKAGED=1"',
      '"%EXE%" "%APP%\\bin\\pvs.js" %*',
      'set "AIBROWSER_EXIT=%ERRORLEVEL%"',
      'endlocal & exit /b %AIBROWSER_EXIT%',
      '',
      ':lazy_start',
      'REM The panel is a GUI window, so nothing starts when the app is installed. The first real',
      'REM command starts it the way that is known to work from agent shells: Start-Process of the app',
      'REM itself, from this cmd.exe process (spawn paths inside the CLI can leave the caller waiting).',
      'REM Commands that need no service (stop/status/packages/help/version) are left alone.',
      'if "%AIBROWSER_NO_AUTO_START%"=="1" exit /b 0',
      'if "%PVS_FIRST%"=="" exit /b 0',
      'if /i "%PVS_FIRST%"=="stop" exit /b 0',
      'if /i "%PVS_FIRST%"=="status" exit /b 0',
      'if /i "%PVS_FIRST%"=="packages" exit /b 0',
      'if /i "%PVS_FIRST%"=="help" exit /b 0',
      'if /i "%PVS_FIRST%"=="-h" exit /b 0',
      'if /i "%PVS_FIRST%"=="--help" exit /b 0',
      'if /i "%PVS_FIRST%"=="version" exit /b 0',
      'if /i "%PVS_FIRST%"=="-v" exit /b 0',
      'if /i "%PVS_FIRST%"=="--version" exit /b 0',
      'set "RUNTIME=%AIBROWSER_RUNTIME%"',
      'if not defined RUNTIME set "RUNTIME=%USERPROFILE%\\.aibrowser"',
      'if not exist "%RUNTIME%" mkdir "%RUNTIME%" 2>nul',
      'REM A state file means a service was started before; let the CLI decide what to do with it.',
      'if exist "%RUNTIME%\\state.json" exit /b 0',
      'set "MODE_ARG=--gui"',
      'if /i "%AIBROWSER_HEADLESS%"=="1" set "MODE_ARG=--headless"',
      'if /i "%AIBROWSER_GUI%"=="0" set "MODE_ARG=--headless"',
      'REM ELECTRON_RUN_AS_NODE must not leak into the app: it would make it run as plain Node.',
      'set "ELECTRON_RUN_AS_NODE="',
      'set "AIBROWSER_LOG_FILE=%RUNTIME%\\gui.log"',
      'powershell -NoLogo -NoProfile -NonInteractive -Command "Start-Process -FilePath \'%EXE%\' -ArgumentList \'--serve\',\'%MODE_ARG%\',\'--json\' -WindowStyle Hidden" >nul 2>nul',
      'set "AIBROWSER_LOG_FILE="',
      'set "AIBROWSER_LAZY_STARTED=1"',
      'exit /b 0',
      '',
    ]
    : [

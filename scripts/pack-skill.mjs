...
async function main() {
  ...
  await fsp.writeFile(path.join(skillDir, 'BUNDLE.md'), [
    ...
    '',
    'bash ~/.agents/skills/aibrowser/scripts/pvs.sh status       # 验证（这条命令不起服务）',
    '```',
    '',
    '安装时**不会**起服务（面板是 GUI 窗口，会打扰用户）：第一次真正干活的命令（`open` / `shot` /',
    '`code` …）会自己把服务拉起来并等到就绪。想自己控制时机/模式时才用启动脚本：',
    '',
    '```bash',
    'bash ~/.agents/skills/aibrowser/scripts/ensure-service.sh          # Linux/macOS/Git Bash',
    '```',
    '',
    '**Windows（PowerShell）**：跑 skill 自带的启动脚本（内部就是「`Start-Process` 拉起应用本体 + 轮询」）：',
    '',
    '```powershell',
    ...
    '',
    '```powershell',
    '$B = "$env:USERPROFILE\\.agents\\skills\\aibrowser\\bundle\\win32-x64"',
    'Start-Process -FilePath "$B\\AIBrowser.exe" -ArgumentList "--serve","--gui","--json" -WindowStyle Hidden   # 无头换成 "--headless"',
    `for ($i = 0; $i -lt 20; $i++) { Start-Sleep -Milliseconds 500; if ((& "$B\\pvs.cmd" status --json) -match '"running":true') { break } }`,
    '```',
    '',
    '`--gui` 要面板窗口、`--headless` 要无头；不想让命令顺手起服务就设 `AIBROWSER_NO_AUTO_START=1`。',
    '',
